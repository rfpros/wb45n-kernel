			   ...
			   size_t bytes)
{
	...
	for (i = 0; i < priv->num_clks; i++) {
		...
		if (ret) {
			...
			mutex_unlock(&at91_fuse_mutex);
			printk(KERN_ERR "failed to prepare/enable clk, error %d\n", ret);
			return ret;
		}
	}
...
static int at91_fuse_probe(struct platform_device *pdev)
{
	...
	priv->base = devm_ioremap_resource(dev, platform_get_resource(pdev, IORESOURCE_MEM, 0));
	if (IS_ERR(priv->base))
		return PTR_ERR(priv->base);

	count = of_clk_get_parent_count(np);
	if (count == 0) {
		printk(KERN_ERR "No clocks in dts\n");
		return -ENODEV;
	}

	priv->clks = devm_kcalloc(dev, count, sizeof(struct clk*), GFP_KERNEL);
	if (!priv->clks)
		return -ENOMEM;

	for (i = 0; i < count; i++) {
		priv->clks[i] = of_clk_get(np, i);
		if (IS_ERR(priv->clks[i])) {
			printk(KERN_ERR "of_clk_get failed with %li\n", PTR_ERR(priv->clks[i]));
